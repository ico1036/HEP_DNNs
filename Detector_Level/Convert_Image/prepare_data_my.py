...
def parse_args():
	...
	add_arg('--GenEvt', default=1, type=int,
			help='The number of Generated evts')
	return parser.parse_args()

def get_tree(files, branch_dict, tree_name, max_events=None):
...
def process_events(tree):
	...
	#print('process step, HT size : ', len(HT))
	#print('process step, fatjetPt size : ', len(fatJetPt))
	#print('process step, fatjetM size : ', len(fatJetM))
	#print('process step, jetM size : ', len(JetM))
...
def filter_delphes_to_numpy(files, max_events=None):
	...
	tree = get_tree(files, branch_dict, tree_name='tree', max_events=max_events)

	if tree is None:
		return None
	# Fix units
	for key in ['clusE', 'clusEM']:
	...
	Lumi = 150000
	xsec = tree['xsec']
	GenN = np.ones(xsec.shape) * args.GenEvt
	print(GenN.shape ,xsec.shape)

	weight = Lumi * xsec / GenN


	#results = process_events(tree)
	results = dict(tree=tree,weight=weight)
...
def get_calo_image(tree, xkey='clusEta', ykey='clusPhi', wkey='clusE',
				   bins=100, xlim=[-4.7, 4.7], ylim=[-3.15, 3.15]):
...
def write_hdf5(filename, outputs):
	...
	with h5py.File(filename, 'w') as hf:
		...
		#	if key in ["hist", "histEM", "histtrack", "passSR4J", "passSR5J", "passSR", "weight"]:
			## --Edited by Jiwoong
			if key in ["hist", "histEM", "histtrack","weight"]:
...
def main():
	...
	output_keys = ['hist', 'histEM', 'histtrack','weight']
